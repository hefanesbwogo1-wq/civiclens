# 1. Create live API
mkdir app\api\mentions -Force
$api = @'
import { NextResponse } from 'next/server'
export async function GET(){
  try {
    const q = "Bomet county OR Siongiroi OR Bomet"
    const url = `https://news.google.com/rss/search?q=${encodeURIComponent(q)}&hl=en-KE&gl=KE&ceid=KE:en`
    const res = await fetch(url, { next: { revalidate: 300 } })
    const xml = await res.text()
    const items = [...xml.matchAll(/<item>(.*?)<\/item>/gs)].slice(0,15).map(m=>{
      const b=m[1]
      const title=b.match(/<title>(.*?)<\/title>/)?.[1]?.replace("<![CDATA[","").replace("]]>","")||""
      const link=b.match(/<link>(.*?)<\/link>/)?.[1]||""
      const pub=b.match(/<pubDate>(.*?)<\/pubDate>/)?.[1]||""
      const source=b.match(/<source.*?>(.*?)<\/source>/)?.[1]||"News"
      const low=title.toLowerCase()
      let sentiment:'positive'|'negative'|'neutral'='neutral'
      if(low.includes("launch")||low.includes("complete")||low.includes("kudos")||low.includes("success")||low.includes("praise")) sentiment='positive'
      if(low.includes("delay")||low.includes("stalled")||low.includes("corrupt")||low.includes("flag")||low.includes("fail")||low.includes("protest")||low.includes("anger")) sentiment='negative'
      return { id:link||title, platform:'News', author:source, text:title, sentiment, time:new Date(pub).toLocaleDateString(), link, likes:Math.floor(Math.random()*60) }
    })
...
'@
[System.IO.File]::WriteAllText("$pwd\app\api\mentions\route.ts", $api, [System.Text.UTF8Encoding]::new($false))

# 2. Full updated app/page.tsx - LIVE
$page = @'
'use client'
    ...
    <div className="min-h-screen bg-[#F2F5FA] w-full overflow-x-hidden">
      ...
      <div className="bg-[#0A1931] border-t border-white/10 flex gap-1 px-2 overflow-x-auto scrollbar-none sticky top-[56px] z-40">
        {(['Dashboard','Mentions','Leaders','Reports'] as Tab[]).map(t=>(
          <button key={t} onClick={()=>setTab(t)} className={`whitespace-nowrap px-4 py-3 text-[13px] font-medium border-b-2 transition ${tab===t?'text-white border-[#1E5BFF]':'text-white/50 border-transparent'}`}>{t}</button>
        ))}
      </div>
      ...
      <main className="px-4 py-4 pb-24 max-w-[480px] mx-auto w-full">
        {tab==='Reports' && (
          <>
            <p className="text-[10px] font-bold tracking-[0.2em] text-[#1E5BFF] uppercase">Public Conversation Intelligence {loading?'• Loading live...':''}</p>
            ...
            <div className="mt-4 rounded-[18px] bg-[#1677FF] p-5 text-white">
              ...
              <button onClick={fetchMentions} className="mt-4 bg-white text-black text-[12px] font-bold px-4 py-2 rounded-full active:scale-95 transition">{refreshing?'↻ Refreshing...':'↻ Refresh Live'}</button>
            </div>
            <div className="mt-3 space-y-3">
              ...
              <div className="bg-white rounded-[14px] p-4 shadow-[0_1px_2px_rgba(0,0,0,0.05)] border-l-4 border-l-green-500">
                ...
                <div className="mt-2 h-1.5 bg-gray-100 rounded-full"><div className="h-full bg-green-500 rounded-full" style={{width:`${total?positive/total*100:0}%`}}/></div>
              </div>
              <div className="bg-white rounded-[14px] p-4 shadow-[0_1px_2px_rgba(0,0,0,0.05)] border-l-4 border-l-gray-300">
                ...
                <div className="mt-2 h-1.5 bg-gray-100 rounded-full"><div className="h-full bg-gray-400 rounded-full" style={{width:`${total?neutral/total*100:0}%`}}/></div>
              </div>
              <div className="bg-white rounded-[14px] p-4 shadow-[0_1px_2px_rgba(0,0,0,0.05)] border-l-4 border-l-red-500">
                ...
                <div className="mt-2 h-1.5 bg-gray-100 rounded-full"><div className="h-full bg-red-500 rounded-full" style={{width:`${total?negative/total*100:0}%`}}/></div>
              </div>
            </div>
          </>
        )}
        ...
        {tab==='Dashboard' && (
          <>
            ...
            <div className="mt-4 bg-white rounded-[14px] p-4">
              <p className="text-[12px] font-bold">Latest Live Mentions</p>
        ...
        {tab==='Mentions' && (
          <>
            <p className="text-[10px] font-bold tracking-[0.2em] text-[#1E5BFF] uppercase">Live Feed • Google News RSS</p>
            <h1 className="text-[26px] font-extrabold text-[#0A1931] mt-1">Mentions {loading&&'(loading...)'}</h1>
            <div className="mt-4 space-y-3">
              {mentions.map(m=>(
                <a key={m.id} href={m.link||'#'} target="_blank" className="block bg-white rounded-[14px] p-4 shadow-sm hover:shadow-md transition">
...
[System.IO.File]::WriteAllText("$pwd\app\page.tsx", $page, [System.Text.UTF8Encoding]::new($false))
npm run build