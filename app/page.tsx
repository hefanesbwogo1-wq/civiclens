...
const FALLBACK: Mention[] = [
  ...
  { id:3, platform:'News', author:'Daily Nation', text:'Governor launches new health initiative', sentiment:'neutral', time:'6h ago', likes:7 },
]
      ...
      <main className="px-4 py-4 pb-24 max-w-[480px] mx-auto w-full">
        ...
        {tab==='Dashboard' && (
          <>
            ...
            <div className="mt-4 bg-white rounded-[14px] p-4"><p className="text-[12px] font-bold">Latest Live</p>{mentions.slice(0,3).map(m=>(<div key={m.id} className="mt-3 text-[12px] border-t pt-3 first:border-0"><b>{m.author}</b>: {m.text}</div>))}</div>
          </>
        )}
        {tab==='Leaders' && (
          <>
            ...
            <div className="mt-4 space-y-3">
              {[{name:'Gov. Hillary Barchok', role:'Governor, Bomet', score:68, mentions:124},{name:'Hon. Linet Chepkorir', role:'Women Rep', score:82, mentions:89}].map(l=>(
